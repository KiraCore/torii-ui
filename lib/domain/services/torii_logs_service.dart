...
class ToriiLogsService {
  ...
  ToriiLogsService(this._repository);
  ...
  Future<LogTxs> fetchTransactionsPerAccount({
    ...
    try {
      ...
      final kiraResponse = await _repository
          .fetchCosmosTransactionsPerAccount<dynamic>(
            ApiRequestModel<QueryLogTxsReq>(
              networkUri: uri,
              requestData: queryLogTxsReq,
              forceRequestBool: true,
            ),
          );
      final ethResponse = await _repository
          .fetchEthTransactionsPerAccount<dynamic>(
            ApiRequestModel<QueryLogTxsReq>(
              networkUri: uri,
              requestData: queryLogTxsReq,
              forceRequestBool: true,
      ...
      QueryLogTxsResp queryLogTxsResp = QueryLogTxsResp.fromJson(
        kiraJson:
            (kiraResponse.data as Map<String, dynamic>)['result']
                as List<dynamic>? ??
            [],
        ethJson:
            (ethResponse.data as Map<String, dynamic>)['result']
                as List<dynamic>? ??
            [],
      ...
      for (final tx in queryLogTxsResp.fromKira) {
        try {
          ...
          getIt<Logger>().w(
            'ToriiLogsService: Skip invalid Kira transaction $tx: $e',
          );
        }
      }
      ...
      for (final tx in queryLogTxsResp.fromEth) {
        try {
          ...
          getIt<Logger>().w(
            'ToriiLogsService: Skip invalid Eth transaction $tx: $e',
          );
        }
      }
      ...
      getIt<Logger>().e(
        'ToriiLogsService: Cannot parse fetchTransactionsPerAccount() for URI $e',
      );
      rethrow;
    }
  }
  ...
  Future<LogTxs> fetchPendingEthTransactions(
    EthereumWalletAddress accountAddress,
  ) async {
    if (!(const bool.hasEnvironment('TORII_LOG_URL'))) {
      throw Exception('Torii log url is not set');
    }
    ...
    try {
      // TODO: correct test transactions
      // if (isTestEnv) {
      //   await Future.delayed(const Duration(seconds: 1));
      //   return _testData;
      // }
      ...
      getIt<Logger>().e(
        'ToriiLogsService: Cannot parse fetchPendingEthTransactions() for URI $e',
      );
      rethrow;
    }
  }
...
PageData<TxListItemModel> _testData = PageData<TxListItemModel>(
  listItems: <TxListItemModel>[
    ...
    TxListItemModel(
      hash:
          '0x3BD165E428985C8FE60A93A9AF0B502F6735F54892FE27425465FAAA04B42BDA',
      ...
      txMsgModels: <MsgSendModel>[
        MsgSendModel(
          fromWalletAddress: AWalletAddress.fromAddress(
            'kira143q8vxpvuykt9pq50e6hng9s38vmy844n8k9wx',
          ),
          toWalletAddress: AWalletAddress.fromAddress(
            '0xb83DF76e62980BDb0E324FC9Ce3e7bAF6309E7b5',
          ),
          tokenAmountModel: TokenAmountModel(
            defaultDenominationAmount: Decimal.fromInt(100),
            tokenAliasModel: TokenAliasModel.local('KEX'),
    ...
    TxListItemModel(
      hash:
          '0x5372F94173105AE3DE4A19CA30A02F1590437F823D45E43EAFC589199C2BC2A2',
      ...
      prefixedTokenAmounts: <PrefixedTokenAmountModel>[
        PrefixedTokenAmountModel(
          ...
          tokenAmountModel: TokenAmountModel.zero(
